...
function formatTextToHTML(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isBullet) {
      if (!inList) {
        processedLines.push('<ul style="margin: 10px 0; padding-left: 25px;">');
        inList = true;
      }
      processedLines.push(`<li style="margin: 5px 0;">${isBullet[1]}</li>`);
    } else {
      if (inList) {
...
function generatePrescriptionHTML(prescriptionContent: string, userId?: string): string {
  ...
  const formattedContent = formatTextToHTML(prescriptionContent);
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p class="subtitle">Generated by Clairify on ${currentDate}</p>
    </div>
...
function generateReportHTML(reportContent: string, userId?: string): string {
  ...
  const formattedContent = formatTextToHTML(reportContent);
